...
query = "CREATE TABLE IF NOT EXISTS music_by_album_name "
query = query + "(year int, artist_name text, album_name text, city text, PRIMARY KEY ((album_name), year))"
try:
...
query = "INSERT INTO music_by_album_name (year, artist_name, album_name, city)"
...
query = """
        select * from music_by_album_name WHERE album_name='Close To You'
"""
try:
...
query = 'drop table music_by_album_name'
try:
    rows = session.execute(query)
except Exception as e:
    print(e)
